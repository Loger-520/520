...
<%
	String username = "";
	username = (String) session.getAttribute("username");//获取会员账号
	if (username == "" || username == null) {//如果没有获取到会员账号，将输出请先登录
		...
		ResultSet rs = conn.executeQuery(
				"select * from tb_order t1,tb_order_detail t2,tb_goods t3 where t2.goodsID=t3.ID and t1.OrderID=t2.orderID and t1.username='"
						+ username + "' order by t1.OrderDate desc");//根据会员账号查询订单信息
		rs.last();
		...
		rs.beforeFirst();
		int orderID = 0;//保存订单号的变量
		int number = 0;//保存商品购买数量的变量
		String recevieName = "";//保存收货人姓名的变量
		String tel = "";//保存收货人手机的变量
		String goodsName = "";//保存产品名称的变量
		float price = 0;//保存消费金额的变量
		String orderDate = "";//保存下单日期的变量
%>
